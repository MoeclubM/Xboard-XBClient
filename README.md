...
```
   ...
   - AdMob 后台“设置并验证回调网址”只用于验证回调可达性，不应该发放真实奖励。验证时用户 ID 和自定义数据可以留空；插件会把不带 `signature`、`transaction_id`、`key_id` 的控制台探测请求识别为验证请求并返回成功。如果 AdMob 页面要求填写，可填：
...
```
     ...
     插件会校验 Google SSV 签名、广告单元和时间戳后返回成功，但不会创建或兑换礼品卡。App 真实看完广告时会使用 `/api/v1/admob/user/config` 下发的用户 ID 和签名 `custom_data`，通过后才发放奖励。
...
- AdMob 控制台验证的不带签名探测请求、空用户 ID/空自定义数据或专用 `xbclient_admob_verify` 只返回验证成功，不创建兑换码、不发奖励。
